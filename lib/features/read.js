...
function isExcelDisabled() {
  const value = String(process.env.MYCOM_DISABLE_EXCEL || "").toLowerCase();
...
async function readExcelFile(filePath, sheetName) {
  if (isExcelDisabled()) {
    throw new Error("Excel reading is disabled by MYCOM_DISABLE_EXCEL. Set it to 0 or unset it to enable.");
  }

  // Load xlsx only when Excel feature is used.
